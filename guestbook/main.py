...
def resource_key():
    ...
    return ndb.Key('Resource', str(day)+'-'+str(month)+'-'+str(year))
...
def reservation_key(email):
    ...
    date = day+'-'+month+'-'+year
    return ndb.Key('Reservation', email+'-'+date)
...
def processAvailableSlots(availableSlots):
    result = ''
    if availableSlots is None:
        return result
    availableSlots.sort(timeComparator)
    for availability in availableSlots:
...
def checkAndModifySlots(resource, startTime, duration):
...
class MainPage(webapp2.RequestHandler):
    def get(self):
        ...
        if user is None:
            ...
            JINJA_ENVIRONMENT.filters['processAvailableSlots'] = processAvailableSlots
...
class AddReservation(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if errorMsg is None:
            resource = Resource.query(Resource.uid == uid).get()
            error = checkAndModifySlots(resource, startTime, duration)
            if error:
...
class ResourceMain(webapp2.RequestHandler):
    def get(self):
        ...
        resource.reservations.sort(timeComparator)
